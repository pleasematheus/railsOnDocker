...
## Como conectar no container Ruby para desenvolvimento utilizando o VSCode
...
4. Configure o arquivo `database.yml` dentro da pasta `config`:
  ...
  > Lembre-se de substituir `<container-postgres>` pelo nome real do container PostgreSQL, que pode ser obtido com `docker ps`.
 > O nome do banco de dados, usuário e senha devem ser configurados de acordo com as suas necessidades e a configuração do container PostgreSQL.
 > O exemplo abaixo é uma configuração básica para o ambiente de desenvolvimento, mas pode ser ajustada conforme necessário.
...
7. Por fim, inicie o servidor Rails:
